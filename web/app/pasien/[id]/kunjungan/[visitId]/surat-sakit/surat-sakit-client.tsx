...
function terbilang(n: number): string {
  // Bahasa Indonesia 1-14 cukup untuk surat sakit (max ~7, buffer sampai 14).
  const tbl = [
    "nol",
    "satu",
    "dua",
...
export function SuratSakitClient({
  patient,
  visit,
  diagnoses,
}: {
  ...
  const endDate = useMemo(
    () => (days > 0 ? addDays(startDate, days - 1) : startDate),
    [startDate, days],
  );
      ...
      <section className="print:hidden mx-auto max-w-3xl px-4 pt-6">
        <div className="rounded-xl border bg-card p-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Pilih lama istirahat dan tanggal mulai — semua tap, tanpa ketik.
          </p>
                ...
                {QUICK_DAYS.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => setDays(n)}
                      className={`inline-flex h-11 min-w-[3.5rem] items-center justify-center rounded-lg border px-3 text-sm font-medium transition-colors ${
                        active
          ...
          <div className="mt-6 space-y-4 text-sm leading-relaxed text-foreground">
            ...
            <p>
              ...
              <span className="font-semibold">
                memerlukan istirahat untuk kesembuhan
              </span>{" "}
              selama{" "}
              <span className="font-semibold">
                {terbilang(days)} ({days}) hari
              </span>
              , terhitung sejak{" "}
              <span className="font-medium">{fmtLong(startDate)}</span> sampai
              dengan <span className="font-medium">{fmtLong(endDate)}</span>.
            </p>
            ...
            {primaryDx ? (
              <p className="text-muted-foreground">
                ...
                <span className="italic text-foreground">
                  {primaryDx.icd10NameId} ({primaryDx.icd10Code})
                </span>
              </p>
            ) : null}
